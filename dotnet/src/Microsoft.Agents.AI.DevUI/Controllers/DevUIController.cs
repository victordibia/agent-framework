...
public class DevUIController : ControllerBase
{
    ...
    public async Task<IActionResult> ExecuteEntity([FromBody] Microsoft.Agents.AI.DevUI.Models.Execution.DevUIExecutionRequest request)
    {
        try
        {
            ...
            // Log ExtraBody for debugging
            _logger.LogInformation("ExtraBody: {ExtraBody}", System.Text.Json.JsonSerializer.Serialize(request.ExtraBody));

            // Extract entity_id from extra_body
            string? entityId = request.GetEntityId();
            _logger.LogInformation("Extracted entity_id: {EntityId}", entityId ?? "NULL");

            if (string.IsNullOrEmpty(entityId))
            {
                return BadRequest(new { error = "entity_id is required in extra_body", extra_body_received = request.ExtraBody });
            }
...
public class StreamingActionResult : IActionResult
{
    ...
    public StreamingActionResult(ExecutionService executionService, string entityId, Microsoft.Agents.AI.DevUI.Models.Execution.DevUIExecutionRequest request, CancellationToken cancellationToken)
    {
        _executionService = executionService;
        _entityId = entityId;
        _request = request;
